 ...
 */
export type AstraEntry =
  | ResolvedStore['outputs'][string]
  | ResolvedStore['inputs'][string]
  | ResolvedStore['decisions'][string]
  | ResolvedStore['findings'][string]
  | ResolvedStore['prior_insights'][string]
  | ResolvedStore['subanalyses'][string];
...
export const PREFIX_TO_TABLE: Record<string, StoreTable> = {
  decision: 'decisions',
  output: 'outputs',
  finding: 'findings',
 ...
 */
export function parseCarrierId(
 ...
 */
export function useEntryByIdentifier(
  identifier: string | undefined,
): AstraEntry | undefined {
  const store = useAstraStore();
  ...
  if (!table) return undefined;
  return (store[table] as Record<string, AstraEntry>)[parsed.id];
}
