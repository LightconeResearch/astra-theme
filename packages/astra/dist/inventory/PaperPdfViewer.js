...
function pdfAssetUrl(baseurl, filename) {
    return `${(baseurl !== null && baseurl !== void 0 ? baseurl : '').replace(/\/+$/, '')}/${filename}`;
}
...
function normalizeQuote(value) {
    ...
    for (const character of value.normalize('NFKC')) {
        if (/\s/.test(character)) {
            if (withSpaces && !withSpaces.endsWith(' '))
                withSpaces += ' ';
            continue;
        }
...
function findQuoteMatch(strings, quote) {
    let flat = '';
    const origin = [];
    ...
    for (const probe of probes) {
        ...
        if (start !== -1)
            return { origin, start, length: probe.length };
    }
    return undefined;
}
function highlightMatch(strings, textDivs, quote) {
    var _a;
    ...
    for (const [itemIndex, range] of [...ranges.entries()].sort((a, b) => a[0] - b[0])) {
        ...
        const raw = (_a = strings[itemIndex]) !== null && _a !== void 0 ? _a : '';
        if (!textDiv)
            continue;
        ...
        firstMark !== null && firstMark !== void 0 ? firstMark : (firstMark = mark);
    }
    return firstMark;
}
function pageStrings(pdf, pageNumber, cache) {
...
export function PaperPdfViewer({ pdfUrl, title, focusRequest, pdfAssetBaseUrl, }) {
    const baseurl = useBaseurl();
    const assetBaseUrl = pdfAssetBaseUrl !== null && pdfAssetBaseUrl !== void 0 ? pdfAssetBaseUrl : baseurl;
    ...
    const quotePageCacheRef = useRef(new Map());
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                if (!disposed)
                    setStatus('The PDF could not be loaded. Use “Open PDF” instead.');
            }
        })();
        return () => {
            disposed = true;
            searchTokenRef.current += 1;
            void (loadingTask === null || loadingTask === void 0 ? void 0 : loadingTask.destroy());
            void (loadedDocument === null || loadedDocument === void 0 ? void 0 : loadedDocument.destroy());
        };
    }, [assetBaseUrl, pdfUrl]);
    useEffect(() => {
        if (!pdf)
        ...
        void (async () => {
            ...
            if (!canvas || !pageElement || !scrollElement || !textLayerElement)
                return;
            try {
                const pdfjs = await loadPdfJs(assetBaseUrl);
                ...
                if (disposed)
                    return;
                if ((pendingHighlight === null || pendingHighlight === void 0 ? void 0 : pendingHighlight.pageNumber) === pageNumber) {
                    const match = highlightMatch(textLayer.textContentItemsStr, textLayer.textDivs, pendingHighlight.quote);
                    if (match) {
        ...
        return () => {
            ...
            textLayer === null || textLayer === void 0 ? void 0 : textLayer.cancel();
        };
    }, [assetBaseUrl, pageNumber, pdf, pendingHighlight, zoomIndex]);
    useEffect(() => {
        if (!pdf || !(focusRequest === null || focusRequest === void 0 ? void 0 : focusRequest.quote))
            return;
        ...
        void (async () => {
            ...
            if (!foundPage) {
                const order = [
                    ...
                    .filter((page) => (page !== undefined && page >= 1 && page <= pdf.numPages))
            ...
            if (token !== searchTokenRef.current)
                return;
            if (!foundPage) {
                if (focusRequest.page !== undefined
    ...
    return (_jsxs("div", { className: "inventory-paper-pdf", "aria-label": `PDF viewer for ${title}`, children: [_jsxs("div", { className: "inventory-paper-pdf__toolbar", children: [_jsxs("div", { className: "inventory-paper-pdf__page-controls", children: [_jsx("button", { type: "button", disabled: !pdf || pageNumber <= 1, onClick: () => changePage(pageNumber - 1), "aria-label": "Previous PDF page", children: "\u2190" }), _jsx("span", { "aria-live": "polite", title: status, children: status }), _jsx("button", { type: "button", disabled: !pdf || pageNumber >= pdf.numPages, onClick: () => changePage(pageNumber + 1), "aria-label": "Next PDF page", children: "\u2192" })] }), _jsxs("div", { className: "inventory-paper-pdf__zoom-controls", children: [_jsx("button", { type: "button", disabled: !pdf || zoomIndex === 0, onClick: () => setZoomIndex((index) => Math.max(0, index - 1)), "aria-label": "Zoom PDF out", children: "\u2212" }), _jsxs("span", { children: [Math.round(ZOOM_LEVELS[zoomIndex] * 100), "%"] }), _jsx("button", { type: "button", disabled: !pdf || zoomIndex === ZOOM_LEVELS.length - 1, onClick: () => setZoomIndex((index) => Math.min(ZOOM_LEVELS.length - 1, index + 1)), "aria-label": "Zoom PDF in", children: "+" })] }), _jsx("a", { href: pdfUrl.split('#')[0], target: "_blank", rel: "noreferrer", "aria-label": "Open PDF in a new tab", children: "Open \u2197" })] }), _jsx("div", { ref: scrollRef, className: "inventory-paper-pdf__scroll", children: _jsxs("div", { ref: pageRef, className: "inventory-paper-pdf__page", children: [_jsx("canvas", { ref: canvasRef }), _jsx("div", { ref: textLayerRef, className: "textLayer inventory-paper-pdf__text-layer" })] }) })] }));
}
//# sourceMappingURL=PaperPdfViewer.js.map